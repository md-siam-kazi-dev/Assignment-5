...
// render priority div color and text dynamic function
const renderPriority = (priority) => {
    let x;
    if(priority === 'high'){
        x = `<div class="px-6 py-1.5 w-fit bg-[#FEECEC] rounded-full text-[14px] text-[#EF4444]">${priority.toUpperCase()}</div>`
    }else if(priority === 'low'){
        x= `<div class="px-6 py-1.5 w-fit bg-[#EEEFF2] rounded-full text-[14px] text-[#9CA3AF]">${priority.toUpperCase()}</div>`
    }else{
...
// render issue quantity function 
...
//render level text and colorr+ icon dynamic 
...
//render modal info dynamic function
